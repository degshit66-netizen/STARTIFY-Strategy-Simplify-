const fs = require('fs');
let content = fs.readFileSync('src/components/ContactsModule.tsx', 'utf8');
...
if (!content.includes('Printer')) {
  content = content.replace(
    "import { Users, Search, PlusCircle, Building2, UserCircle, Edit3, Trash2 } from 'lucide-react';",
    "import { Users, Search, PlusCircle, Building2, UserCircle, Edit3, Trash2, Printer } from 'lucide-react';"
  );
}
...
content = content.replace(
  /<div className="flex items-center gap-2 self-start sm:self-auto bg-zinc-100 dark:bg-zinc-800 px-3 py-1\.5 rounded-xl text-xs font-bold text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">\s*<span>\{contacts\.length\} network connections<\/span>\s*<\/div>/,
  `<div className="flex items-center gap-2 self-start sm:self-auto bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-xl text-xs font-bold text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">
          <span>{contacts.length} network connections</span>
        </div>
...
fs.writeFileSync('src/components/ContactsModule.tsx', content);
console.log('patched Contacts');
