const fs = require('fs');
let content = fs.readFileSync('src/components/InventoryModule.tsx', 'utf8');
...
if (!content.includes('Printer')) {
  content = content.replace(
    "import { Package, Search, PlusCircle, AlertTriangle, Edit3, Trash2 } from 'lucide-react';",
    "import { Package, Search, PlusCircle, AlertTriangle, Edit3, Trash2, Printer } from 'lucide-react';"
  );
}
...
content = content.replace(
  /<div className="flex items-center gap-2 self-start sm:self-auto bg-zinc-100 dark:bg-zinc-800 px-3 py-1\.5 rounded-xl text-xs font-bold text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">\s*<span>\{deduped\.length\} items logged<\/span>\s*<\/div>/,
...
fs.writeFileSync('src/components/InventoryModule.tsx', content);
