const fs = require('fs');
let content = fs.readFileSync('src/components/AuditTrailModule.tsx', 'utf8');
...
if (!content.includes('Printer')) {
  content = content.replace(
    "import { ShieldCheck, Search, Activity, User, Monitor, Clock, FileText, Download } from 'lucide-react';",
    "import { ShieldCheck, Search, Activity, User, Monitor, Clock, FileText, Download, Printer } from 'lucide-react';"
  );
}

content = content.replace(
  /<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">/,
  `<PrintHeader title="System Audit Trail" />
...
content = content.replace(
  /<div className="flex items-center gap-2 self-start sm:self-auto bg-zinc-100 dark:bg-zinc-800 px-3 py-1\.5 rounded-xl text-xs font-bold text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">\s*<span>\{logs\.length\} unalterable records<\/span>\s*<\/div>/,
  `<div className="flex items-center gap-2 self-start sm:self-auto bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-xl text-xs font-bold text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">
          <span>{logs.length} unalterable records</span>
        </div>
...
fs.writeFileSync('src/components/AuditTrailModule.tsx', content);
console.log('patched AuditTrail');
